...
var planeColor = "#d32f2f";
var planeColors = ["#d32f2f", "#337ab7", "#AAAAAA"];
var pointColor = "#000000";
var planesActive = false;
var linesActive = true;
var pointsActive = true;
  ...
  for (var i = 0; i < gameObjects.length; i++) {
    ...
    for (var j = 0; j < gameObjects.length; j++) {
      ...
      if (dist <= lineDistance) {
        ...
        if (planesActive) {
          ctx.fillStyle = planeColors[Math.floor(i % planeColors.length)];
          ctx.globalAlpha = Math.acos(Math.abs(dist / lineDistance)-.1);
          ctx.beginPath();
